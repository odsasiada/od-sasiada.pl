...
- Dostarczone wcześniej, zweryfikowane wg sprint-1.md: `pnpm dev` startuje, seed + place-order przechodzą na schemacie bez inventory. Brak dedykowanego skryptu spike dla tej historyjki.
- Pokryte **pośrednio** przez `tests/integration/order-placement.integration.test.ts` (place-order na schemacie bez inventory), reprodukowalne przez `pnpm test` (live Docker Postgres `od-sasiada-pg`) → cała suita **39 passed, 1 skipped**. Złożenie zamówienia przechodzące na schemacie bez kolumn `inventory` potwierdza, że wyłączenie inwentarza nie blokuje przepływu zamówień.
