...
- `tests/integration/order-detail-idor.integration.test.ts` — regresja IDOR zamrażająca kontrakt izolacji (patrz niżej).
...
Regresja `tests/integration/order-detail-idor.integration.test.ts` → **4 przypadki IDOR**, reprodukowalne przez `pnpm test` (live Docker Postgres `od-sasiada-pg`) → cała suita **39 passed, 1 skipped**. Test tworzy klientów A1, A2 (tenant A) i B1 (tenant B) z jednym zamówieniem każdy, następnie uruchamia zapytanie strony dla każdego scenariusza:
